...
You can create your own Apptainer/Singularity containers, customized for your personal project needs. We suggest you [install](install-apptainer.html) locally and create images on a local machine where you have root access. 

 It is also possible to create your images directly on Spiedie, without further installing Apptainer on a local machine. 
 ...
 5. [Host your container online](#apptainer_push)
...
## <a name="access_token"></a> Create a Access Token
 ...
 ```bash
 cd 
 echo [paste token here] >> .apptainer/sylabs-token
 ```
...
## <a name="using_recipe"></a> Using a recipe to build 
 ...
 ``` bash
 apptainer build --remote image.sif definition_file.def 
 ```
...
Apptainer allows you to create PGP keys and use them to sign and verify containers. This ensures an added layer of security while sharing containers to 
...
## <a name="apptainer_push"></a> Host your container online
...
You can push your built contier using the push command on Apptainer. 
...
```bash
apptainer push -U image.sif library://your-username/project-dir/my-container:tags
```
